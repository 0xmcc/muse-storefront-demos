...
| D15 | **Optimise perceived speed, not generation speed.** Cache every look; pre-style the rest of the rail in the background after the first result lands | Marko's brief, 2026-08-06. Generation is ~16s and won't get faster. Returning to a seen garment is instant (no network), and by the second pick the next is usually already cached — exploration feels like filters, not a workflow | 2026-08-06 |
...
| 47 | A ~16s model cannot feel "nearly instantaneous" on its own | TD | CONTRACT (D15) | Solved by caching + background pre-styling, not by a faster model |
